...
import redis
from metric_simulator.metric_service import MetricService, MAX_RETRIES, RETRY_DELAY
...
@pytest.fixture
def metric_service(mock_llm_repository, mock_metric_repository, mock_simulator_repository):
...
async def test_generate_metric_data(metric_service):
    ...
    result = await metric_service.generate_metric_data(mock_metric_generator, metric_name, llm_name)

    assert result == [1, 2, 3]
    mock_metric_generator.generate_data_points.assert_called_once_with(metric_name)

@pytest.mark.asyncio
async def test_simulate_data_points_with_retry_success(metric_service, mock_redis_client):
    # Arrange
    mock_lock = AsyncMock()
    mock_redis_client.redis.lock.return_value.__enter__.return_value = mock_lock

    with patch.object(metric_service, 'simulate_data_points', new_callable=AsyncMock) as mock_simulate:
...
@pytest.mark.asyncio
async def test_simulate_data_points_with_retry_lock_error(metric_service, mock_redis_client):
...
@pytest.mark.asyncio
async def test_simulate_data_points(metric_service, mock_llm_repository, mock_metric_repository,
                                    mock_simulator_repository, mock_redis_client):
    mock_llm = Mock()
    mock_llm.id = 1
    ...
    with patch('metric_simulator.metric_service.MetricGenerator') as mock_generator_class:
        mock_generator = Mock()
        ...
        mock_simulator_repository.bulk_add_metrics.assert_called_once_with(1, 1, [1, 2, 3])
