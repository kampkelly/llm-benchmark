...
@pytest.fixture
def benchmark_service(mock_llm_repository, mock_metric_repository, mock_simulator_repository):
...
def test_get_simulation_and_rankings_from_repositories(benchmark_service, mock_redis_client, mock_metric_repository,
    ...
    mock_simulator_repository.get_metric_means_by_llm.return_value = [("LLM1", 0.854321)]
...
def test_get_simulation_and_rankings_by_metric_name_from_cache(benchmark_service, mock_redis_client):
    metric_name = "metric1"
    cached_data = {"metric1": [{"llm_name": "LLM1", "mean_value": 0.85}]}
    mock_redis_client.redis.exists.return_value = True
    mock_redis_client.redis.get.return_value = json.dumps(cached_data)
...
def test_get_simulation_and_rankings_by_metric_name_from_repositories(benchmark_service, mock_redis_client,
                                                                      mock_metric_repository, mock_simulator_repository):
    metric_name = "metric1"
    mock_redis_client.redis.exists.return_value = False
    mock_metric = Mock()
...
def test_get_simulation_and_rankings_by_metric_name_not_found(benchmark_service, mock_metric_repository):
