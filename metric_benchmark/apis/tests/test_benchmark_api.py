...
def test_get_simulation_and_rankings_by_metric_name(mock_benchmark_service):
    ...
    mock_benchmark_service.get_simulation_and_rankings_by_metric_name.return_value = mock_response

    response = client.get(f"/rankings/{metric_name}", headers={"X-API-Key": TEST_API_KEY})

    assert response.status_code == 200
    assert response.json() == mock_response
    mock_benchmark_service.get_simulation_and_rankings_by_metric_name.assert_called_once_with(metric_name)
